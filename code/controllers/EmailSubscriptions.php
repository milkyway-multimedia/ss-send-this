...
class EmailSubscriptions extends Controller
{
    ...
    public function block($r)
    {
        ...
        return $controller->customise([
            ...
            'Content' => '<p>' . _t('SendThis.UNSUBSCRIBE-CONTENT',
                    'Enter your email address below if you wish to no longer receive any emails from <a href="{url}">{site}</a>',
                    [
                        'url'  => singleton('director')->absoluteBaseURL(),
                        'site' => class_exists('SiteConfig') ? singleton('SiteConfig')->current_site_config()->Title : singleton('LeftAndMain')->ApplicationName,
                    ]) . '</p>',
            'Form'    => $this->BlockForm(),
    ...
    public function unblock($r)
    {
        ...
        return $controller->customise([
            ...
            'Content' => '<p>' . _t('SendThis_Controller.ALLOW_EMAIL-CONTENT',
                    'Enter your email address below to allow email communications from <a href="{url}">{site}</a>', [
                        'url'  => singleton('director')->absoluteBaseURL(),
                        'site' => class_exists('SiteConfig') ? singleton('SiteConfig')->current_site_config()->Title : singleton('LeftAndMain')->ApplicationName,
                    ]) . '</p>',
    ...
    public function doBlock($data, $form, $r)
    {
        ...
        if (!Blacklist::get()->filter('Email', $fields['Email'])->exists()) {
            if (Email::mailer() instanceof Mailer) {
                Email::mailer()->eventful()->fire(singleton('sendthis-event')->named('sendthis:blacklisted', Email::mailer()), '',
                    $fields['Email'],
                    ['message' => _t('SendThis.BLACKLIST_BY_USER', 'Unsubscribe by user'), 'internal' => true],
                    $fields);
            }
            ...
            return $this->respondToFormAppropriately([
                    'message' => _t(
                        ...
                        [
                            'email'   => $fields['Email'],
                            'unblock' => $this->Link('unblock/' . str_replace(['.', '-'], ['%2E', '%2D'],
    ...
    public function doUnblock($data, $form, $request)
    {
        ...
        if (Email::mailer() instanceof Mailer) {
            Email::mailer()->eventful()->fire(singleton('sendthis-event')->named('sendthis:whitelisted', \Email::mailer()), '',
                $fields['Email'], [
                    'message'  => _t('SendThis.WHITELIST_BY_USER', 'User has requested to be whitelisted'),
                    'internal' => true,
                ], $fields);
        }
    ...
    {
        $templates = ['SendThis', 'Page', 'ContentController'];
    ...
    {
        return class_exists('FormBootstrapper') && !($form instanceof \FormBootstrapper) ? \FormBootstrapper::create($form) : $form;
    }
}
